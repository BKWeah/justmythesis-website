...
function normalizeProjectType(value?: string | null): ProjectType | null {
  ...
  switch (normalized) {
    case 'A - Proposal Development':
    case 'Proposal Development':
      return 'Proposal Development';

    case 'B - Foundation Thesis':
    case 'Foundation Thesis':
      return 'Foundation Thesis';

    case 'C - Complete Thesis Development':
    case 'Complete Thesis Development':
      return 'Complete Thesis Development';

    case 'D - Thesis Rescue':
    case 'Thesis Rescue':
      return 'Thesis Rescue';

    case 'E - Formatting & Compliance':
    case 'Formatting & Compliance':
      return 'Formatting & Compliance';

    case 'F - Defense Presentation':
    case 'Defense Presentation':
      return 'Defense Presentation';
...
export async function GET(
  ...
  try {
    ...
    if (requestError || !requestData) {
      console.error('Request detail fetch error:', requestError);
    ...
    const recommendation =
      recommendationResult.status === 'fulfilled'
        ? recommendationResult.value.data || null
        : null;

    const notes = rawNotes.map((note: any) => ({
      ...note,
      author_name: note.author_name || 'Staff',
    }));

    const activities = rawActivities.map((activity: any) => ({
      ...activity,
      performer_name: activity.performer_name || 'System',
    }));
  ...
  try {
    ...
    switch (action) {
      ...
      case 'assign_staff': {
        ...
        const { data: assignedStaff } = await adminClient
          .from('staff_users')
          .select('full_name')
          .eq('id', staffId)
          .single();
        ...
        await adminClient.from('activity_logs').insert({
          ...
          description: `Assigned to ${
            assignedStaff?.full_name || 'staff member'
          }`,
          support_request_id: id,
          entity_type: 'support_requests',
          entity_id: id,
      ...
      case 'convert_to_project': {
        ...
        const { data: recommendation, error: recommendationError } =
          await adminClient
            .from('recommendations')
            .select('id, status, recommended_service')
            .eq('support_request_id', id)
            .maybeSingle();
        ...
        if (!recommendation || recommendation.status !== 'Accepted') {
          return NextResponse.json(
            {
              error:
                'Project conversion is available only after the client accepts the recommendation.',
            },
            { status: 409 }
          );
        }
        ...
        if (existingProject) {
          return NextResponse.json(
            {
              error: `This request has already been converted to project ${existingProject.project_reference}.`,
        ...
        const approvedService = normalizeProjectType(
          service_package || recommendation.recommended_service
        );
        ...
        if (!approvedService) {
          return NextResponse.json(
            {
              error:
                'The selected service package is not supported by the projects table.',
            },
            { status: 400 }
          );
        }
        ...
        const { data: project, error: projectError } =
          await adminClient
            ...
            .insert({
              ...
              approved_service: approvedService,
              start_date,
              expected_delivery_date: deadline || null,
              status: 'Project Activated',
              completion_percentage: 0,
              notes: `Created from support request ${requestData.request_reference || id}.`,
            })
            .select('*')
            .single();
        ...
        const { error: documentLinkError } = await adminClient
          .from('documents')
          .update({ project_id: project.id })
          .eq('support_request_id', id);

        if (documentLinkError) {
          console.error('Document project-linking error:', documentLinkError);
        }
        ...
        const { error: activityError } = await adminClient
          ...
          .insert({
            ...
            description: `Project ${project.project_reference} created from request`,
            client_id: requestData.client_id,
            support_request_id: id,
        ...
        if (activityError) {
          console.error('Project activity log error:', activityError);
        }
