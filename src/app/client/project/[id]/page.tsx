...
export default function ClientProjectDetailsPage() {
  ...
  const [project, setProject] =
    useState<ClientProjectDetail | null>(null);
  const [deliverables, setDeliverables] = useState<
    ClientDeliverable[]
  >([]);

  const [isLoading, setIsLoading] = useState(true);
  const [isDeliverablesLoading, setIsDeliverablesLoading] =
    useState(true);
  const [downloadingId, setDownloadingId] =
    useState<string | null>(null);

  const [error, setError] = useState<string | null>(null);
  const [deliverablesError, setDeliverablesError] =
    useState<string | null>(null);
  ...
  const fetchProject = useCallback(async () => {
    try {
      ...
      const data =
        (await response.json()) as ClientProjectDetailResponse;
      ...
      if (!response.ok || !data.project) {
        throw new Error(
          data.error || 'Unable to retrieve this project.'
        );
      }
  ...
  const fetchDeliverables = useCallback(async () => {
    try {
      ...
      const data =
        (await response.json()) as ClientDeliverablesResponse;
      ...
      if (!response.ok) {
        throw new Error(
          data.error || 'Unable to retrieve deliverables.'
        );
      }
  ...
  const handleDownload = async (
    ...
    try {
      ...
      const response = await fetch(
        `/api/client/projects/${encodeURIComponent(
          projectIdentifier
        )}/deliverables?deliverableId=${encodeURIComponent(
          deliverable.id
        )}`,
        {
          method: 'GET',
      ...
      const data =
        (await response.json()) as ClientDownloadResponse;
      ...
      if (!response.ok || !data.url) {
        throw new Error(
          data.error || 'Unable to download this file.'
        );
      }
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-6 py-10">
          ...
          >
            <div className="flex min-h-72 flex-col items-center justify-center text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Please wait while we retrieve your project.
              </p>
            </div>
          </Card>
        )}
        ...
        {!isLoading && !error && project && (
          <>
            ...
            >
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Download files released by the JUSTmyTHESIS
                    team.
                  </p>
                </div>
                ...
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => void fetchDeliverables()}
                ...
                deliverables.length === 0 && (
                  <div className="mt-6 flex min-h-56 flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 px-6 text-center">
                    ...
                    <h3 className="mt-4 font-semibold text-foreground">
                      No deliverables released yet
                    </h3>

                    <p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">
                      Files released by the JUSTmyTHESIS team
                      will appear here when they are ready.
                    </p>
                  </div>
                )}
                ...
                deliverables.length > 0 && (
                  <div className="mt-6 space-y-4">
                    {deliverables.map((deliverable) => (
                      <div
                        key={deliverable.id}
                        className="flex flex-col gap-5 rounded-xl border border-gray-200 p-5 lg:flex-row lg:items-center lg:justify-between"
                      >
                        ...
                        <Button
                          type="button"
                          onClick={() =>
                            void handleDownload(deliverable)
                          }
                          disabled={
            ...
            <ProjectMessages projectId={projectIdentifier} />
          </>
        )}
      </div>
    </main>
