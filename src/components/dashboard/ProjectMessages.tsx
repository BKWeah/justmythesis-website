...
export function ProjectMessages({
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || 'Unable to retrieve project messages.'
        );
      }
    ...
    <Card variant="bordered" padding="lg">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          <MessageSquareText
            className="mt-1 h-5 w-5 text-brand-green"
            aria-hidden="true"
          />
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Communicate with the JUSTmyTHESIS team about this
              project.
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex min-h-48 flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 px-6 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Send the first message about this project.
            </p>
          </div>
        ) : (
              ...
              <article
                ...
                className={`rounded-xl border p-4 ${
                  message.isCurrentUser
                    ? 'ml-auto border-brand-green/30 bg-brand-green/5'
